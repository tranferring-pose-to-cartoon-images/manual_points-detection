...
from modules.manual_points_detector import *
...
def transfer_one(generator, kp_detector, source_image, driving_video, transfer_params):
    print("Ali is in the transfer_one")
    ...
    print("kp_source['mean'].size(): ", kp_source['mean'].size())
    print("kp_source['var'].size(): ", kp_source['var'].size())
    print("kp_source", kp_source)

    t= (64,64)
    spatial_size = spatial_size = np.array(t)[np.newaxis, np.newaxis]


    heatmaps=kp2gaussian(kp_source, (64,64), kp_variance='matrix')


    # ===========================================================================
    # MANUAL POINT DETECTION
    kp_manual = manual_point_detector("spiddy.png")
    kp_manual = np.asarray(kp_manual)
    kp_manual = kp_manual.reshape(1,1,kp_manual.shape[0],-1)
    print(kp_manual.shape)
    kp_manual = torch.from_numpy(kp_manual)
    kp_manual = kp_manual.cuda()
    kp_manual = kp_manual.type(torch.cuda.FloatTensor)
    # transforming keypoints
    # ================================
    t = (64, 64)
    spatial_size = spatial_size = np.array(t)[np.newaxis, np.newaxis]
    spatial_size = torch.from_numpy(spatial_size)
    spatial_size = spatial_size.cuda()
    spatial_size = spatial_size.type(torch.cuda.FloatTensor)
    # kp_array = spatial_size * (kp_array + 1) / 2
    kp_manual = (kp_manual/spatial_size)*2-1
    kp_source['mean'] = kp_manual
    #==========================================================================
    kp_zeros_variance = torch.zeros([1,1,10,2,2], dtype = torch.float)
    kp_zeros_variance = torch.randn(1,1,10,2,2) # random SD=1, mean = 0
    kp_zeros_variance = kp_zeros_variance.cuda()
    print(kp_zeros_variance.size())

    kp_source['var'] = kp_zeros_variance
    # ==========================================================================
    ...
    # HERE IS THE KP DETECTOR FOR VIDEO AND SOURCE IMAGE
